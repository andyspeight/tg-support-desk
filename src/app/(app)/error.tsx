...
// Catches any unhandled error from a page render or a server action across the
// agent desk and shows a recoverable card instead of a hard 500. Nothing the
// agent typed is lost (the composer keeps its own draft); this is the last-line
// safety net behind the per-action handling.
export default function DeskError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Desk error boundary:", error);
  }, [error]);
    ...
    <div className="flex min-h-[70vh] items-center justify-center p-6">
      <div className="w-full max-w-md rounded-xl border border-line bg-surface p-6 text-center shadow-sm">
        ...
        <p className="mt-1.5 text-sm text-ink-2">
          That didn’t go through. Nothing was lost — try again, or head back to the inbox.
        </p>
